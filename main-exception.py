...
class ExcelGUI:
    ...
    def gui(self):
        ...
        self.generate = Button(self.frame_1, text='Генерирай', font=(self.font_1, 12), bd=2,
                               ...
                               command=self.generate_btn).place(x=235, y=360, width=120)

        self.cancel = Button(self.frame_1, text='Изтрий', font=(self.font_1, 12), bd=2, command=self.cancel_btn,
                               cursor="hand2", bg=self.color_3, fg=self.color_4).place(x=365, y=360, width=120)
    ...
    def search(self):
        self.excel_file = f"{pathlib.Path().resolve()}\data\last-shtatno.xlsx"
        self.workbook = load_workbook(self.excel_file)
        self.sheet = self.workbook.active

        for row_range in [(9, 39), (44, 51), (57, 66)]:
    ...
    def generate_btn(self):
        ...
        try:
            self.replacements = {
                '<<id>>': self.id_var.get(),
                '<<signature_date>>': str(date(2024, 2, 13).strftime("%d.%m.%Y")),
                ...
                "<<ProsVr>>": self.professional_experience_percent_var.get(),
...
try:
    ...
    messagebox.showerror("Greshka", RuntimeError+"\nError-7")
    raise RuntimeError

else:
    messagebox.showinfo("OK", "OK7")